...
export default function HomeScreen({ navigation }) {
  ...
  useEffect(() => { loadExpenses(); }, [isFocused]);

  const loadExpenses = async () => {
    try {
      const data = await AsyncStorage.getItem("expenses");
      if (data) setExpenses(JSON.parse(data));
    } catch (error) { console.log("Error loading expenses:", error); }
  };
  ...
  const editExpense = (expense) => navigation.navigate("Add Expense", { expense });
  ...
  const sections = [
    { key: "Cash", data: categorized.cash, color: "#9ACD32", total: totalCash },
    { key: "Credit Card", data: categorized.credit, color: "#78c4dfff", total: totalCredit },
    { key: "Debit Card", data: categorized.debit, color: "#e78deaff", total: totalDebit },
  ];

  const renderSection = ({ item }) => (
    <View style={[styles.section, { backgroundColor: item.color }]}>
...
const styles = StyleSheet.create({
  ...
  expenseItem: { flexDirection: "row", justifyContent: "space-between", backgroundColor: "rgba(255,255,255,0.3)", borderRadius: 10, padding: 10, marginBottom: 8 },
